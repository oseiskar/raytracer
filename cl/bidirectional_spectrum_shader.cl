...
__kernel void bidirectional_spectrum_shader(
        ...
        // 0 or 1, tells if emittance can be summed to this ray
        //global uint *may_emit,
        ...
        constant float4 *rvecs_cmask_and_light_point)
{
    ...
    float3 r = ray[gid];
    float3 n = normal[gid];
    const float3 rvec = rvecs_cmask_and_light_point[0].xyz;
    float3 gauss_rvec = rvecs_cmask_and_light_point[1].xyz;
    
    float cur_prob = 0;
    float cur_mult = 1.0;
    float blur;
    
    const float3 cmask = rvecs_cmask_and_light_point[2].xyz;
    ...
    if (p >= cur_prob)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                const float3 light_point = rvecs_cmask_and_light_point[3].xyz;
                float3 shadow_ray = light_point - pos[gid];
                
                if (dot(shadow_ray, n) > 0) {
                    const float shadow_dist = length(shadow_ray);
                    shadow_ray = fast_normalize(shadow_ray);
                    
                    float diff_density = 2.0 * dot(n,shadow_ray);
                    img[gid] += diff_density*intensity[gid]*cmask*cur_mult*shadow_mask[gid];
                }
