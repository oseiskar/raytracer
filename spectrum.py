...
class Spectrum:
    ...
    def __init__(self, wavelength_range_nm = None, resolution = None):
        ...
        if wavelength_range_nm is None: wavelength_range_nm = (360,830)
    ...
    def interp1d(self, x, data_x, data_y):
        if len(data_y.shape) == 1: data_y = data_y[np.newaxis,:]
        # TODO: scipy.interp1d ?
        return np.array([np.interp(x, data_x, data_y[i,:]) \
            for i in range(data_y.shape[0])])
    
    def map_left(self, wavelengths, y ):
        return self.interp1d(self.wavelengths, wavelengths, y)
    
    def map_right(self, wavelengths, y ):
        return self.interp1d(wavelengths, self.wavelengths, y)
    
    def cie_1931_xyz(self):
        if not hasattr(self, 'cie_xyz'):
            # Google for "CIE 1931 data" to find an XLS with this data
            cie_data = np.genfromtxt('cie-1931.csv', delimiter=',')
    ...
    def black_body(self, T, normalized=True):
        ...
        l = self.wavelengths * 1e-9
        energy_density = 2*h*c**2 / l**5 * 1.0 / (np.exp(h*c / (l*k*T)) - 1.0)
        if normalized: energy_density /= np.max(energy_density)
        return energy_density
    
    def linear_dispersion_ior(self, n_d, abbe_number):
        alpha = (n_d - 1.0) / ((486.1 - 656.3) * abbe_number)
    ...
    def get_color(self, density, brightness = None):
        ...
        if brightness is not None:
            ...
            if isinstance(brightness, int): c = c.astype(int)
        return tuple(c)
